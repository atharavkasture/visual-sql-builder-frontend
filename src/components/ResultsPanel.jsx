...
// import { Play, Save, Download, DatabaseZap, BarChart2, Table } from 'lucide-react';
// import SavedQueries from './SavedQueries';
// import QueryChart from './QueryChart';

// const ResultsPanel = ({ result, isLoading, onRunQuery, queryConfig, onConfigChange }) => {
//     const [showSaved, setShowSaved] = useState(false);
//     const [viewMode, setViewMode] = useState('table'); // State for 'table' or 'chart'

//     const isDDL = queryConfig.queryType === 'DDL';

//     // Logic to determine if a chart can be shown. Requires at least one string and one number column.
...
//     // Effect to reset the view to 'table' whenever a new result comes in
//     useEffect(() => {
//         setViewMode('table');
//     }, [result]);
...
//     // Function to save the current query config to local storage
//     const saveQuery = () => {
...
//                                 <Table size={16} />
//                             </button>
//                             <button onClick={() => setViewMode('chart')} className={`p-1 rounded ${viewMode === 'chart' ? 'bg-white shadow-sm' : ''}`} title="Chart View">
//                                 <BarChart2 size={16} />
//                             </button>
//                         </div>
//                     )}
//                 </div>
//                 <div className="flex items-center gap-2">
//                     <button onClick={onRunQuery} disabled={isLoading} className="btn-primary">
//                         <Play size={16} />
...
import SavedQueries from './SavedQueries';
import QueryChart from './QueryChart'; // Import the chart component

const ResultsPanel = ({ result, isLoading, onRunQuery, queryConfig, onConfigChange }) => {
    const [showSaved, setShowSaved] = useState(false);
    const [viewMode, setViewMode] = useState('table'); // 'table', 'bar', or 'pie'

    const isDDL = queryConfig.queryType === 'DDL';

    // Check if the data is suitable for charting
    const canShowChart = result && result.data && result.data.length > 0 &&
        Object.values(result.data[0]).some(val => typeof val === 'number') &&
        Object.values(result.data[0]).some(val => typeof val === 'string');
    
    // Reset to table view whenever a new query result comes in
    useEffect(() => { 
        setViewMode('table'); 
    }, [result]);
    ...
    return (
        <div className="h-full flex flex-col bg-white">
            <div className="flex-shrink-0 p-2 border-b border-slate-200 flex items-center justify-between bg-slate-50">
                <div className="flex items-center gap-2">
                    <h3 className="font-semibold text-sm text-slate-700">Actions & Results</h3>
                    {/* NEW: Chart toggle now includes Pie chart */}
                    {canShowChart && (
                        <div className="flex items-center bg-slate-200 rounded-md p-0.5">
                            <button onClick={() => setViewMode('table')} className={`p-1 rounded ${viewMode === 'table' ? 'bg-white shadow-sm' : ''}`} title="Table View">
                                <Table size={16} />
